# -*- coding: utf-8 -*-
"""
Created on Wed Mar 19 14:09:30 2025
...
def find_similar_materials(material_name, df_Material, df_Process_Caracteristic, df_Supplier, constraints, material_index_formula, material_index_maxmin, price_increase_percentage):
...
def add_distance_to_material (material_list, main_address, geolocator,df_Material,df_Process_Caracteristic,df_Supplier):
    df = pd.DataFrame({
    ...
    for i in range(len(df)):
        ...
        df.loc[i, 'Distance'] = shorter_distance(material, main_address, geolocator,df_Material,df_Process_Caracteristic,df_Supplier)[0]
        df.loc[i, 'Supplier_name'] = shorter_distance(material, main_address, geolocator,df_Material,df_Process_Caracteristic,df_Supplier)[1]# Set the Distance value for the first row
    return (df)
...
def find_closest_match(search_term, database, incineration, landfill):
    ...
    if best_match:
        print('fuzzy match')
        rer_activities = [a for a in valid_activities if a.as_dict()['name'] == best_match.as_dict()['name'] and a.as_dict().get("location", "") == "RER"]
        if rer_activities:
            best_match= rer_activities[0]

        row_activities = [a for a in valid_activities if a.as_dict()['name'] == best_match.as_dict()['name'] and a.as_dict().get("location", "") == "RoW"]
        if row_activities:
            best_match= row_activities[0]

        glo_activities = [a for a in valid_activities if a.as_dict()['name'] == best_match.as_dict()['name'] and a.as_dict().get("location", "") == "GLO"]
        if glo_activities:
...
def find_similar_time(new_process, process,df_Material,df_Process_Caracteristic,df_Supplier, mat1,mat2):
    ...
    efficiency_1 = df_Process_Caracteristic.loc[df_Process_Caracteristic['Name'] == process[0], 'Average efficiency'].iloc[0]
    efficiency_2=df_Process_Caracteristic.loc[df_Process_Caracteristic['Name'] == new_process, 'Average efficiency'].iloc[0]
...
def find_similar_process(process_list, i,df_Material,df_Process_Caracteristic,df_Supplier):
    ...
    process_type = df_Process_Caracteristic.loc[df_Process_Caracteristic['Name'] == process_list[0], 'Process type'].iloc[0]
    ...
    df_Process_Caracteristic['Tolerance min'] = pd.to_numeric(df_Process_Caracteristic['Tolerance min'], errors='coerce')
    ...
    df_Process_Caracteristic['Section thickness min'] = pd.to_numeric(df_Process_Caracteristic['Section thickness min'], errors='coerce')
    df_Process_Caracteristic['Section thickness max'] = pd.to_numeric(df_Process_Caracteristic['Section thickness max'], errors='coerce')
    df_Process_Caracteristic['Roughness max'] = pd.to_numeric(df_Process_Caracteristic['Roughness max'], errors='coerce')
    df_Process_Caracteristic['Roughness min'] = pd.to_numeric(df_Process_Caracteristic['Roughness min'], errors='coerce')
...
def give_Process_Material(similar, process_list,df_Material,df_Process_Caracteristic,df_Supplier,base_mat):
    ...
    for i in range(1, len(similar)):  # Skip the base material
        ...
        for process in process_list:
            ...
            new_processes = find_similar_process(process, material,df_Material,df_Process_Caracteristic,df_Supplier)  # Replace with your logic
            print('new_process')
            print(new_processes)
            ...
            if len(new_processes)>0:                
                ...
                for new_process in separated_new_process_list:
                    print('new_process')
                    print(new_process)
                    new_time = find_similar_time(new_process[0], process,df_Material,df_Process_Caracteristic,df_Supplier,base_mat, material)   
...
def Create_Modules_Subproduct(material_name, main_address, geolocator, database, database_bis, material_id,material_volume, process_list,df_Material,df_Process_Caracteristic,df_Supplier,constraints,material_index_formula, material_index_maxmin,increase_percentage_price):
    ...
    similar_material_table,similar=find_similar_materials(material_name, df_Material,df_Process_Caracteristic,df_Supplier,constraints,material_index_formula, material_index_maxmin,increase_percentage_price)
    
    print('similar_material_table')
    print(similar_material_table)
    df_Material_and_Distance = add_distance_to_material(similar,main_address, geolocator,df_Material,df_Process_Caracteristic,df_Supplier)
    ...
    df_Material_And_Process = give_Process_Material(similar, process_list,df_Material,df_Process_Caracteristic,df_Supplier,material_name)
    ...
    for i in similar:
        ...
        material_price_index=similar_material_table.loc[similar_material_table["Material"] == i, "Price per kilo"].values[0]
        ...
        similar_process_list = df_Material_And_Process.loc[df_Material_And_Process["Material"] == i, "Process"].values[0]
        ...
        for process, time, base_process in zip(similar_process_list, time_list, process_list):
            ...
            for process_alternative, time_alternative in zip(process, time):
                ...
                print(amount)
                module_process.new_exchange(input=process_alternative_act, amount=amount, type='technosphere',unit='min').save()
                module_process.new_exchange(input=module_process, amount=1, type='production',unit='unit').save()
                power=df_Process_Caracteristic.loc[df_Process_Caracteristic["Name"] == str(process_alternative[0]), "Power"].values[0]
        ...
        existing_process = next((act for act in database_bis if act['name'] == f"{i} from {supplier_name} fully processed"), None)
        ...
        module_material.new_exchange(input=incineration, amount=incineration_amount, type='technosphere',unit='unit').save()
        module_material.new_exchange(input=landfill, amount=landfill_amount, type='technosphere',unit='unit').save()
        module_material.new_exchange(input=transport_waste, amount=1/1000*70, type='technosphere',unit='unit').save()
        
        module_material.save()
        material_price=material_price_index*material_mass
        new_row = {"Sub_Product_name": module_material['name'], "Quality": material_quality_index, "Price": material_price}
        ...
        'outputs': [
            (('database_bis', unique_code), material_id, 1)
        ],
...
module=Create_Modules_Subproduct('Styrene Acrylonitrile',main_address, geolocator, database, database_bis, 'M001',2,[('Press brake', 2.5, 'Flat', None, None, None, 5,'M001')],df_Material,df_Process_Caracteristic,df_Supplier, constraints,material_index_formula, material_index_maxmin,increase_percentage_price )
print (type(module))


def Create_Modules_Subproduct_2030(material_name, main_address, geolocator, database_2030, database_bis_2030, material_id,material_volume, process_list,df_Material,df_Process_Caracteristic,df_Supplier,constraints,material_index_formula, material_index_maxmin,increase_percentage_price):
    
    material_volume = float(material_volume)
    print(material_volume)
    #The scaling is for the material selection
    scaling=['None',10,10,'None','None','None']
    similar = find_similar_materials(material_name, df_Material,df_Process_Caracteristic,df_Supplier,constraints,material_index_formula, material_index_maxmin,increase_percentage_price)[1]
    ...
    for i in similar:
        ...
        for process, time, base_process in zip(similar_process_list, time_list, process_list):
            ...
            for process_alternative, time_alternative in zip(process, time):
                ...
                'name': name,
                'outputs': [
                    (('database_bis_2030', name), output, 1)
                ],
        ...
        if supplier_name.size > 0:
            ...
            supplier_name = "Unknown Supplier"
        existing_process = next((act for act in database_bis_2030 if act['name'] == f"{i} from {supplier_name} fully processed"), None)
        ...
        'name': module_material['name'],
        'outputs': [
            (('database_bis_2030', unique_code), material_id, 1)
        ],
...
def Create_Modules_Subproduct_2050(material_name, main_address, geolocator, database_2050, database_bis_2050, material_id,material_volume, process_list,df_Material,df_Process_Caracteristic,df_Supplier,constraints,material_index_formula, material_index_maxmin,increase_percentage_price):
    ...
    for i in similar:
        ...
        for process, time, base_process in zip(similar_process_list, time_list, process_list):
            ...
            for process_alternative, time_alternative in zip(process, time):
                ...
                'name': name,
                'outputs': [
                    (('database_bis_2050', name), output, 1)
                ],
        ...
        if supplier_name.size > 0:
            ...
            supplier_name = "Unknown Supplier"
        existing_process = next((act for act in database_bis_2050 if act['name'] == f"{i} from {supplier_name} fully processed"), None)
        ...
        'name': module_material['name'],
        'outputs': [
            (('database_bis_2050', unique_code), material_id, 1)
        ],
