...
import os
from material_treatment_data import Create_Modules_Subproduct, Create_Modules_Subproduct_2030, Create_Modules_Subproduct_2050
...
def Create_all_Modules(
    ...
    print("NEW MATERIAL")
    columns = ["Sub_Product_name", "Quality"]
    ...
    for material_id, material_info in product["materials"].items():
        ...
        for routing_id in material_info["routing"]:
            matches_process = difflib.get_close_matches(
                product["routing"][routing_id]["name"], process_names, n=1, cutoff=0.5
            )
            process_list.append(
                (
                    matches_process[0],
                    product["routing"][routing_id]["time"],
                    product["routing"][routing_id]["shape"],
                    product["routing"][routing_id]["tolerance"],
                    product["routing"][routing_id]["thickness"],
                    product["routing"][routing_id]["roughness"],
        ...
        sub_product_module, key, micro_quality_df, micro_energy = Create_Modules_Subproduct(
            matches[0],
            main_address,
            geolocator,
            database,
    ...
    if existing_process:
        ...
        Final_module = database_bis.new_node(
            code=product["specificity"]["id"], name=product["specificity"]["id"], unit="unit"
        )
        print(Final_module)
        Final_module.save()
    for i, j, k in zip(material, quantity, material_id_list):
        ...
        Final_module.new_exchange(
            input=bd.get_activity(("database_bis", i)), amount=j, type="technosphere", unit="unit"
        ).save()
    ...
    raw_data.append(
        {
            "name": "Assembly",
            "outputs": [(Final_module.key, product["specificity"]["id"], 1.0)],
...
def Create_all_Modules_2030(
    ...
    for material_id, material_info in product["materials"].items():
        ...
        sub_product_module, key = Create_Modules_Subproduct_2030(
            matches[0],
            main_address,
            geolocator,
            database_2030,
    ...
    if existing_process:
        ...
        # Create a new activity
        Final_module = database_bis_2030.new_node(
            code=product["specificity"]["id"], name=product["specificity"]["id"], unit="unit"
        )
        print(Final_module)
        Final_module.save()
    for i, j, k in zip(material, quantity, material_id_list):
        ...
        Final_module.new_exchange(
            input=bd.get_activity(("database_bis_2030", i)), amount=j, type="technosphere", unit="unit"
        ).save()
...
def Create_all_Modules_2050(
    ...
    for material_id, material_info in product["materials"].items():
        ...
        sub_product_module, key = Create_Modules_Subproduct_2050(
            matches[0],
            main_address,
            geolocator,
            database_2050,
    ...
    if existing_process:
        ...
        # Create a new activity
        Final_module = database_bis_2050.new_node(
            code=product["specificity"]["id"], name=product["specificity"]["id"], unit="unit"
        )
        print(Final_module)
        Final_module.save()
    for i, j, k in zip(material, quantity, material_id_list):
        ...
        Final_module.new_exchange(
            input=bd.get_activity(("database_bis_2050", i)), amount=j, type="technosphere", unit="unit"
        ).save()
...
raw_data = []
rd, quality_df, energy = Create_all_Modules(
    database,
    database_bis,
    product,
    raw_data,
...
raw_data_2030 = []
rd_2030 = Create_all_Modules_2030(
    database_2030,
    database_bis_2030,
    product,
    raw_data_2030,
...
raw_data_2050 = []
rd_2050 = Create_all_Modules_2050(
    database_2050,
    database_bis_2050,
    product,
    raw_data,
